...
datatype
  wnode = WNODE of (ref(pos), ref(wxopt)) where wxopt = Option(wnode)
...
//
fun
wnode_make_pos(xy: pos): wnode = WNODE(ref(xy), ref(None()))
//
fun
wnode_make_pos_tail(xy: pos, wx: wnode): wnode = WNODE(ref(xy), ref(Some(wx)))
//
(* ****** ****** *)
//

implement
worm_remove_last(wx0) =
  (
    if wx0.length() >= 2
      then ignoret(worm_takeout_last(wx0)) 
      else ()
  // end of [if]
  )
...
implement
worm_takeout_last(wx0) = 
  let
    fun loop(wx0: wnode, wx1: wnode) : wnode = 
      let
        val opt = wx1.tail()
      in
        case+ opt of
          | Some(wx2) => loop(wx1, wx2)
          | None((*void*)) => let val () = wx0.tail(None) in wx1 end
      end
...
worm_move(dir, wx) = 
  ...
  in
    ...
    if(n0 = 1)
      then 
        let val () = wx.pos(xy) in wx end // end of [then]
    else 
      let val wy = worm_takeout_last(wx) in wy.pos(xy); wy.tail(Some(wx)); wy end // end of [else]
...
theWorm_move_if(dir) = 
  ...
  in
    ...
    ifcase
      | knd = Blank => theWorm_move(dir)
      | knd = Path => theWorm_move(dir)
      | knd = Apple => theWorm_movex(dir)
      | knd = Obstacle => theWorm_remove_last() 
      | knd = Worm => theWorm_remove_last() 
      | _(*else*) => ()
