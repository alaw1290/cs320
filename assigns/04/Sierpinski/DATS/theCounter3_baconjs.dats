...
extern
fun
draw_Sierpinski_canvas_show(count: int): void = "mac#"
//
(* ****** ****** *)
...
//
    | Next() =>
      let val res2 = res + 1 in
        if res2 >= 120 then res2-120 else res2
      end // end of [Next2]
    | Next2() =>
      let val res2 = res + 2 in
        if res2 >= 120 then res2-120 else res2
      end // end of [Next2]
//
    | Prev() =>
      let val res2 = res - 1 in
        if res2 >= 0 then res2 else 120 + res2
      end // end of [Prev]
    | Prev2() =>
      let val res2 = res - 2 in
        if res2 >= 0 then res2 else 120 + res2
...
//
    | Rand() => double2int(120*JSmath_random())
//
...
in (* in-of-local *)
//
val () = theCounts.onValue()(lam(count) => draw_Sierpinski_canvas_show(count))
//
end // end of [local]
